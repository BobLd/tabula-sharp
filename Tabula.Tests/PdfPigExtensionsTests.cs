...
{
    public class PdfPigExtensionsTests
    {
        #region TestBasicExtractor tests
        ...
        {
            using (PdfDocument document = PdfDocument.Open("Resources/m27.pdf", new ParsingOptions() { ClipPaths = true }))
            {
                var page = document.GetPage(1);

                var tables = page.GetTables(new[] { new PdfRectangle(28.28, 532 - (103.04 - 79.2), 732.6, 532) }, new BasicExtractionAlgorithm()).ToArray();
        ...
        {
            using (PdfDocument document = PdfDocument.Open(ARGENTINA_DIPUTADOS_VOTING_RECORD_PDF, new ParsingOptions() { ClipPaths = true }))
            {
                var page = document.GetPage(1);

                var tables = page.GetTables(new[] { new PdfRectangle(12.75, 55, 557, 567) }, new BasicExtractionAlgorithm()).ToArray();
                Table table = tables[0];
        ...
        public void TestVerticalRulingsPreventMergingOfColumns()
        {
            ...
            using (PdfDocument document = PdfDocument.Open("Resources/campaign_donors.pdf", new ParsingOptions() { ClipPaths = true }))
            {
                var page = document.GetPage(1);

                var tables = page.GetTables(new[] { new PdfRectangle(40.43, 755 - (398.76 - 255.57), 557.35, 755) }, new BasicExtractionAlgorithm(rulings)).ToArray();
                Table table = tables[0];
        ...
        public void TestExtractColumnsCorrectly()
        {
            ...
            {
                using (PdfDocument document = PdfDocument.Open(EU_002_PDF, new ParsingOptions() { ClipPaths = true }))
                {
                    var page = document.GetPage(1);

                    var tables = page.GetTables(new[] { new PdfRectangle(70.0, 725 - (233 - 115), 510.0, 725) }, new BasicExtractionAlgorithm()).ToArray();
                    Table table = tables[0];
        ...
        public void TestExtractColumnsCorrectly2()
        {
            if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows)) // || RuntimeInformation.IsOSPlatform(OSPlatform.Linux))
            {
                using (PdfDocument document = PdfDocument.Open(EU_017_PDF, new ParsingOptions() { ClipPaths = true }))
                {
                    var page = document.GetPage(3);

                    PageArea pageArea = UtilsForTesting.GetPage(EU_017_PDF, 3); // TODO - does not make sense to do like that - for further dev

                    var tables = page.GetTables(new[] { new PdfRectangle(148.44, 543 - (711.875 - 299.625), 452.32, 543) }, new BasicExtractionAlgorithm(pageArea.VerticalRulings)).ToArray();
                    Table table = tables[0];
        ...
        {
            using (PdfDocument document = PdfDocument.Open(FRX_2012_DISCLOSURE_PDF, new ParsingOptions() { ClipPaths = true }))
            {
                var page = document.GetPage(1);

                var tables = page.GetTables(new[] { new PdfRectangle(48.09, 563, 551.89, 685.5) }, new BasicExtractionAlgorithm()).ToArray();
                Table table = tables[0];
                var result = UtilsForTesting.TableToArrayOfRows(table);
        ...
        {
            using (PdfDocument document = PdfDocument.Open("Resources/12s0324.pdf", new ParsingOptions() { ClipPaths = true }))
            {
                var page = document.GetPage(1);

                var tables = page.GetTables(new[] { new PdfRectangle(17.25, 342, 410.25, 560.5) }, new BasicExtractionAlgorithm()).ToArray();
                Table table = tables[0];
                var rows = table.Rows;
                var firstRow = rows[0];
        ...
        {
            using (PdfDocument document = PdfDocument.Open("Resources/us-017.pdf", new ParsingOptions() { ClipPaths = true }))
            {
                var page = document.GetPage(2);

                PageArea pageArea = UtilsForTesting.GetPage("Resources/us-017.pdf", 2).GetArea(new PdfRectangle(90, 97, 532, 352)); // TODO - Does not make sense

                var tables = page.GetTables(new[] { new PdfRectangle(90, 97, 532, 352) }, new BasicExtractionAlgorithm(pageArea.VerticalRulings)).ToArray();
                Table table = tables[0];
                IReadOnlyList<Cell> cells = table.Cells;
                foreach (var rectangularTextContainer in cells)
                {
                    System.Diagnostics.Debug.Print(rectangularTextContainer.GetText());
                }
        ...
        public void TestRealLifeRTL2()
        {
            ...
            using (PdfDocument document = PdfDocument.Open("Resources/indictb1h_14.pdf", new ParsingOptions() { ClipPaths = true }))
            {
                var page = document.GetPage(1);

                var tables = page.GetTables(new[] { new PdfRectangle(120.0, 842 - 622.82, 459.9, 842 - 120.0) }, new BasicExtractionAlgorithm()).ToArray();
                Table table = tables[0];
        ...
        public void TestEmptyRegion()
        {
            using (PdfDocument document = PdfDocument.Open("Resources/indictb1h_14.pdf", new ParsingOptions() { ClipPaths = true }))
            {
                ...
                var tables = page.GetTables(new[] { new PdfRectangle(0, 700, 100.9, 800) }, new BasicExtractionAlgorithm()).ToArray();
                Table table = tables[0];
        ...
        public void TestTableWithMultilineHeader()
        {
            ...
            using (PdfDocument document = PdfDocument.Open("Resources/us-020.pdf", new ParsingOptions() { ClipPaths = true }))
            {
                var page = document.GetPage(2);

                var tables = page.GetTables(new[] { new PdfRectangle(35.0, 151, 560, 688.5) }, new BasicExtractionAlgorithm()).ToArray();
                Table table = tables[0];
